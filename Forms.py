...
class UpdateAdminForm(Form):
    ...
    authority = SelectMultipleField('Authority:',
                                    choices=[('Products', 'Products'), ('Event', 'Event'),
                                             ('Store Locator', 'Store Locator'),
                                             ('Services', 'Services'), ('Book', 'Book'), ('Tips', 'Tips'), ('Manage Admin', 'Manage Admin')],
                                    option_widget=widgets.CheckboxInput(),
                                    widget=widgets.ListWidget(prefix_label=False)
                                    )
    description = TextAreaField('Description:', [validators.Length(min=1, max=500)])
...
def validate_not_past(form, field):
    today = date.today()
    if field.data < today:
        raise validators.ValidationError('Booking cannot be a past date')

def validate_no_special_symbols(form, field):
    if any(char in string.punctuation for char in field.data):
        raise ValidationError("Special symbols are not allowed.")
class BookingForm(FlaskForm):
            machine_id = StringField('Machine ID:', render_kw={'readonly': True})
            status = StringField('Status:', render_kw={'readonly': True})
            model = StringField('Model:', render_kw={'readonly': True})
            capacity = StringField('Capacity:', render_kw={'readonly': True})  # Using StringField for display
            price_per_use = StringField('Price:', render_kw={'readonly': True})  # Using StringField for display

            date = DateField('Date:', validators=[DataRequired(), validate_not_past])
            time_slot = SelectField('Time Slot:', choices=[
                ('9AM-10AM', '9AM-10AM'),
                ('10AM-11AM', '10AM-11AM'),
                ('11AM-12PM', '11AM-12PM'),
                ('12PM-1PM', '12PM-1PM')], validators=[DataRequired()])
            submit = SubmitField('Add to Cart')

class LaundryMachineForm(FlaskForm):
            status = SelectField('Status:', choices=[
                ('Available', 'Available'),
                ('Under Maintenance', 'Under Maintenance')], validators=[DataRequired()])
            model = UpperStringField('Model:',
                                     validators=[DataRequired(), Length(max=200), validate_no_special_symbols])
            capacity = DecimalField('Capacity (kg):', validators=[DataRequired(), NumberRange(min=0, max=10)])
            price_per_use = DecimalField('Price Per Use:', validators=[DataRequired(), NumberRange(min=0, max=10)])
            submit = SubmitField('Add Machine')

class CheckoutForm(FlaskForm):
    customer_email = EmailField('Customer Email:', validators=[DataRequired(), Email()])
    first_name = UpperStringField('First Name:', validators=[DataRequired(), Length(min=2, max=50)])
    last_name = UpperStringField('Last Name:', validators=[DataRequired(), Length(min=2, max=50)])
    ...
    cvv = StringField('CVV:', validators=[DataRequired(), Length(min=3, max=3, message='CVV must be a 3-digit number.'), lambda form, field: cvv_validator(form, field)])
    expiry_date_day = SelectField('Expiry Day:', choices=[(str(day), str(day)) for day in range(1, 32)], validators=[DataRequired()])
    expiry_date_month = SelectField('Expiry Month:', choices=[(str(month), str(month)) for month in range(1, 13)], validators=[DataRequired()])
    email_notification = SelectField('Email Notifications:', choices=[('yes', 'Yes'), ('no', 'No')], validators=[DataRequired()])
    submit = SubmitField('Complete Checkout')

def cvv_validator(form, field):
    if not re.fullmatch(r'\d{3}', field.data):
