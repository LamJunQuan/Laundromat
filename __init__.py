...
def update_profile_page():
    ...
    if request.method == 'POST' and update_user_form.validate():
        ...
        return render_template('update_profile_page.html', form=update_user_form , registrations=user_registrations)
...
def view_registrations():
    ...
    try:
        ...
        user_registrations = {event_name: user_ids for event_name, user_ids in all_registrations.items() if user_id in user_ids}
    except Exception as e:
        print(f"Error in retrieving registrations: {e}")
        user_registrations = {}
    finally:
...
def add_machine():
    print("Entered add_machine route")  # Debug statement
    machine_form = LaundryMachineForm(request.form)

    if request.method == 'POST':
        print("Form submitted")  # Debug statement

        if machine_form.validate():
            print("Form validation successful")  # Debug statement
            print("Form data:", machine_form.data)  # Debug statement

            db_machines = shelve.open('machines.db', 'c')
            machines_dict = db_machines.get('Machines', {})

            # Generate a unique machine ID
            machine_id = "M" + str(len(machines_dict) + 1)

            # Create a LaundryMachine object without specifying the image path
            machine = LaundryMachine(
                machine_id,
                machine_form.status.data,
                machine_form.model.data,
                machine_form.capacity.data,
                machine_form.price_per_use.data
            )

            machines_dict[machine_id] = machine
            db_machines['Machines'] = machines_dict
            db_machines.close()

            return redirect(url_for('machine_cart'))  # Redirect to the home page or any other page

        else:
            print("Form validation failed")  # Debug statement
            # This will print the specific validation errors
            for field, errors in machine_form.errors.items():
                print(f"Error in the field '{field}': {', '.join(errors)}")

    return render_template('add_machine.html', form=machine_form)


# retrieve machine part

@app.route('/retrieve_machine')
def machine_cart():
    machine_dict = {}
    db = shelve.open('machines.db', 'c')  # Open the machines database
    machine_dict = db.get('Machines', {})  # Retrieve machines
    db.close()

    machine_list = [machine_dict[key] for key in machine_dict]

    return render_template('retrieve_machine.html', count=len(machine_list), machine_list=machine_list)
...
def update_machine(machine_id):
    ...
    # Convert the machine_id to string, in case it's not already
    machine_id = str(machine_id)

    machine = machines_dict.get(machine_id)
    if not machine:
        db_machines.close()
        return 'Machine not found', 404  # Machine not found in the database

    update_machine_form = LaundryMachineForm(request.form)

    if request.method == 'POST' and update_machine_form.validate():
        # Update machine details
        machine.set_status(update_machine_form.status.data)
        machine.set_model(update_machine_form.model.data)
        machine.set_capacity(update_machine_form.capacity.data)
        machine.set_price_per_use(update_machine_form.price_per_use.data)

        # Save the changes
        machines_dict[machine_id] = machine
        db_machines['Machines'] = machines_dict
        db_machines.close()
        return redirect(url_for('machine_cart'))
    elif request.method == 'GET':
        # Pre-populate the form with the current machine details
        update_machine_form.status.data = machine.get_status()
        update_machine_form.model.data = machine.get_model()
        update_machine_form.capacity.data = machine.get_capacity()
        update_machine_form.price_per_use.data = machine.get_price_per_use()

    db_machines.close()
    return render_template('update_machine.html', form=update_machine_form, machine_id=machine_id)


# delete machine part

@app.route('/delete_machine/<machine_id>', methods=['POST'])
def delete_machine(machine_id):
    db_machines = shelve.open('machines.db', 'w')

    machines_dict = db_machines.get('Machines', {})

    if machine_id in machines_dict:
        del machines_dict[machine_id]  # Remove the machine entry
        db_machines['Machines'] = machines_dict  # Save the updated dict
    ...
    flash(message)  # Optionally flash a message to the user
    return redirect(url_for('machine_cart'))  # Redirect to the machine listing page
...
def book():
    ...
    return render_template('book_page.html', count=len(machine_list), machine_list=machine_list)


@app.route('/maintenance_page')
def maintence_page():
    return render_template('maintenance_page.html')


@app.route('/timings/<machine_id>/<status>/<model>/<capacity>/<price_per_use>/', methods=['GET', 'POST'])
def create_booking(machine_id, status, model, capacity, price_per_use):
    ...
    create_booking_form = BookingForm(request.form)

    if request.method == 'GET':
        create_booking_form.machine_id.data = machine_id
        create_booking_form.status.data = status
        create_booking_form.model.data = model
        create_booking_form.capacity.data = capacity
        create_booking_form.price_per_use.data = price_per_use

    if request.method == 'POST' and create_booking_form.validate_on_submit():
        db_sales = shelve.open('sales.db', 'c')
        sales_record = db_sales.get('SalesRecord', {})

        for sale in sales_record.values():
            if sale.get_machine_id() == machine_id and sale.get_date() == create_booking_form.date.data and sale.get_time() == create_booking_form.time_slot.data:
                flash('This machine has an outstanding sales issue for the selected date and time.', 'error')
                return render_template('timings.html', form=create_booking_form)

        db_cart = shelve.open('cart.db', 'c')
        cart_dict = db_cart.get('Cart', {})

        for booking in cart_dict.values():
            if booking.get_machine_id() == machine_id and booking.get_date() == create_booking_form.date.data and booking.get_time_slot() == create_booking_form.time_slot.data:
                flash('This time slot is already booked. Please select a different time.', 'error')
                return render_template('timings.html', form=create_booking_form)

        booking_id = len(cart_dict) + 1
        new_booking = Booking(booking_id, create_booking_form.date.data, create_booking_form.time_slot.data, machine_id, status, model, capacity, price_per_use)
        cart_dict[booking_id] = new_booking
        db_cart['Cart'] = cart_dict
...
def update_cart_item(id):
    ...
    error_message = None
    update_cart_form = BookingForm(request.form)

    if request.method == 'POST' and update_cart_form.validate():
        try:
            with shelve.open('cart.db', 'w') as db_cart:
                cart_dict = db_cart.get('Cart', {})

                machine_id = update_cart_form.machine_id.data
                new_date = update_cart_form.date.data
                new_time_slot = update_cart_form.time_slot.data

                # Check if the updated booking conflicts with any existing ones, except itself
                for key, existing_booking in cart_dict.items():
                    if (existing_booking.get_machine_id() == machine_id and
                        existing_booking.get_date() == new_date and
                        existing_booking.get_time_slot() == new_time_slot and key != id):
                        error_message = "A booking with the same machine, date, and time slot already exists."
                        break

                if not error_message:
                    booking = cart_dict.get(id)
                    if booking:
                        booking.set_date(new_date)
                        booking.set_time_slot(new_time_slot)
            ...
            app.logger.error('Error in updating booking: %s', str(e))

    elif request.method == 'GET':
        try:
            with shelve.open('cart.db', 'r') as db_cart:
                cart_dict = db_cart.get('Cart', {})
                booking = cart_dict.get(id)
                if booking:
                    update_cart_form.date.data = booking.get_date()
                    update_cart_form.time_slot.data = booking.get_time_slot()
                    update_cart_form.machine_id.data = booking.get_machine_id()
                    # Populate other form fields if necessary, similar to below
                    update_cart_form.status.data = booking.get_status()
                    update_cart_form.model.data = booking.get_model()
                    update_cart_form.capacity.data = booking.get_capacity()
                    update_cart_form.price_per_use.data = booking.get_price_per_use()
        except Exception as e:
            error_message = f"An error occurred: {str(e)}"
            app.logger.error('Error in retrieving booking for update: %s', str(e))

    return render_template('updateCart.html', form=update_cart_form, id=id, error_message=error_message)

# ... Rest of your Flask app code ...



# Include your login route and other routes as needed...
...
def shopping_cart():
    ...
    user_id = session['user_id']
    cart_dict = {}
    total_price = 0.0  # Initialize total price as a float for accurate calculations

    try:
        db_cart = shelve.open('cart.db', 'r')
        cart_dict = db_cart.get('Cart', {})
        db_cart.close()
    except Exception as e:
        flash('Error in retrieving items from cart.', 'error')
        app.logger.error('Error in retrieving items from cart: %s', str(e))

    cart_list = [booking for booking in cart_dict.values()]
    total_price = sum(float(booking.get_price_per_use()) for booking in cart_list)

    return render_template('shoppingCart.html', count=len(cart_list), cart_list=cart_list, total_price=total_price)
    ...
    try:
        with shelve.open('cart.db', 'w') as db_cart:
            ...
            if id in cart_dict:
                ...
                print(f"Item with ID {id} not found in cart.")  # For debugging

        flash('Booking deleted successfully!', 'success')
    except Exception as e:
        flash(f"An error occurred while deleting booking: {str(e)}", 'error')
        app.logger.error('Error in deleting booking: %s', str(e))
...
def checkout():
    ...
    try:
        with shelve.open('cart.db', 'r') as db_cart:
            ...
            for booking in cart_dict.values():
                cart_list.append(booking)
                total_price += float(booking.get_price_per_use())
    except Exception as e:
        error_message = 'Error retrieving items from cart.'
        app.logger.error(f"Error: {e}")

    if request.method == 'POST' and checkout_form.validate_on_submit():
        try:
            # Transfer cart items to sales records
            with shelve.open('sales.db', 'c') as db_sales:
                for booking in cart_list:
                    sale_id = str(len(db_sales) + 1)
                    sale = Sale(
                        sale_id,
                        booking.get_machine_id(),
                        booking.get_date(),
                        booking.get_time_slot(),
                        booking.get_price_per_use()
                    )
                    db_sales[sale_id] = sale

            # Save checkout details for administrative purposes
            with shelve.open('checkout_info.db', 'c') as db_checkout_info:
                checkout_id = str(len(db_checkout_info) + 1)
                checkout = Checkout(
                    checkout_id,
                    checkout_form.customer_email.data,
                    checkout_form.first_name.data,
                    checkout_form.last_name.data,
                    checkout_form.card_number.data,
                    checkout_form.billing_address.data,
                    checkout_form.cvv.data,
                    checkout_form.expiry_date_day.data,
                    checkout_form.expiry_date_month.data,
                    checkout_form.email_notification.data
                )
                db_checkout_info[checkout_id] = checkout

            # Clear the cart after transferring items to sales
            with shelve.open('cart.db', 'w') as db_cart:
                db_cart['Cart'] = {}  # Reset the cart
    ...
    return render_template('checkout.html', form=checkout_form, error_message=error_message, cart_list=cart_list, total_price=total_price)

@app.route('/confirmation')
def confirmation():
    if 'user_id' not in session:
        flash('You must be logged in to view the confirmation page.', 'warning')
        return redirect(url_for('login'))
...
@app.route('/admin_checkout/', methods=['GET'])
def admin_checkout():
    checkout_info_list = []
    sales_list = []

    try:
        with shelve.open('checkout_info.db', 'r') as db_checkout_info:
            checkout_info_list = list(db_checkout_info.values())
            app.logger.debug("Checkout Info List: %s", checkout_info_list)  # Log the checkout info for debugging
    except Exception as e:
        app.logger.error("Error occurred in retrieving checkout info: %s", e)

    try:
        with shelve.open('sales.db', 'r') as db_sales:
            sales_list = list(db_sales.values())
            app.logger.debug("Sales List: %s", sales_list)  # Log the sales list for debugging
            if sales_list:
                app.logger.debug("Type of first sale item: %s", type(sales_list[0]))  # Log the type of the first item in sales list
                app.logger.debug("First sale item: %s", sales_list[0])  # Log the first item in sales list for detailed inspection
    except Exception as e:
        app.logger.error("Error occurred in retrieving sales records: %s", e)

    return render_template('Admin_checkout.html',
                           checkout_info_count=len(checkout_info_list),
                           checkout_info_list=checkout_info_list,
...
def sales_data():
    ...
    daily_sales_count = defaultdict(int)  # Change to count items

    try:
        with shelve.open('sales.db', 'r') as db_sales:
            for sale_id, sale in db_sales.items():
                sale_date = sale.get_date()
                if isinstance(sale_date, datetime):
                    sale_date = sale_date.date()

                if start_date <= sale_date <= end_date:
                    daily_sales_count[sale_date] += 1  # Increment count

    except Exception as e:
        app.logger.error(f"Error retrieving sales data: {e}")
        return jsonify({"error": "Error retrieving sales data"}), 500

    sorted_dates = sorted(daily_sales_count.keys())
    dates = [date.strftime("%Y-%m-%d") for date in sorted_dates]
    counts = [daily_sales_count[date] for date in sorted_dates]

    return jsonify(dates=dates, sales=counts)  # Return counts
